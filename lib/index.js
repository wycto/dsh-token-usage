...
export function apply(ctx) {
  ...
  // ---------- RPC (static-plugin: 经 ctx.connection 挂到 /api 共享通道) ----------
  // dsh rc.6 静态插件没有 harness 全局; host→client 通信走 connection 服务:
  //   host:  ctx.connection.rpc.intercept('/api', matches, handler, { authority })
  //   client: ctx.connection.rpc.call('/api', endpoint, payload)
  // handler 返回 RpcResult: { ok: true, data } | { ok: false, error }
  function rpcOk(data) { return { ok: true, data } }
  function rpcErr(message) { return { ok: false, error: { code: 'internal', message, details: {} } } }

  const connection = ctx.get('connection')
  if (connection && connection.rpc && typeof connection.rpc.intercept === 'function') {
    ctx.effect(() => connection.rpc.intercept(
      '/api',
      (endpoint) => endpoint.startsWith('token-usage/'),
      async (endpoint, payload) => {
        try {
          const method = endpoint.slice('token-usage/'.length)
          if (method === 'query') {
          ...
          if (method === 'export') {
            ...
            return rpcOk({ csv: lines.join('\n'), count: list.length })
          }
          if (method === 'scan') {
            await scanHistory()
            return rpcOk({ total: records.length })
          }
          if (method === 'hello') {
            return rpcOk({ ok: true, records: records.length, scanned: scannedSessions.size })
          }
          return rpcErr('unknown endpoint: ' + endpoint)
        } catch (e) {
          return rpcErr((e && e.message) || String(e))
        }
      },
      { authority: 'loopback' }
    ), 'dsh-token-usage: /api interceptor')
  }
