...
export function apply(ctx) {
  ...
  // USD→CNY 估算汇率: 默认 7.2, 可被 settings.yaml 的 token-usage.usdCnyRate 覆盖
  let usdCnyRate = 7.2
  try {
    const settings = ctx.get('settings')
    if (settings && typeof settings.get === 'function') {
      const v = settings.get('token-usage')
      if (v && typeof v === 'object' && typeof v.usdCnyRate === 'number' && v.usdCnyRate > 0) usdCnyRate = v.usdCnyRate
    }
  } catch (e) {
    console.error('[token-usage] read usdCnyRate failed, use default 7.2', e && e.message)
  }
          ...
          if (method === 'query') {
            ...
            return sendJson(res, 200, {
              ...
              data: {
                ...
                models: [...new Set(all.map((r) => r.model).filter(Boolean))],
                modelsByProvider,
