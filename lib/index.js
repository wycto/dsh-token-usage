    ...
    wsCtx.effect(() => wsCtx.webServer.register({
      ...
      async handler(req, res) {
        try {
          ...
          if (method === 'scan') {
            await scanHistory()
            return sendJson(res, 200, { ok: true, data: { total: records.length } })
          }
          if (method === 'hello') {
            return sendJson(res, 200, { ok: true, data: { ok: true, records: records.length, scanned: scannedSessions.size } })
          }
